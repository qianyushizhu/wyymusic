<template>
  ...
  <div style="background-color: rgb(43,43,43);color:#fff;height: 3000px;">
        <!--    左侧区域 播放评论 -->
...
<!--            评论区域-->
            <el-row>
                <p style="margin-top: 15px;font-size: 18px;margin-bottom: 10px;">评论({{commentTotal}})</p>

                <h4 style="font-weight: 300" v-if="queryInfo.offset === 0 && videoHotComment.length !== 0">精彩评论</h4>
                <!--精彩评论的信息-->
                <div v-for="(item,index) in videoHotComment"
                :key="index"
                    ...
                    <div>
                        ...
                        <div style="margin-left: 60px;margin-top: -60px;">
                            ...
                            <div v-if="item.beReplied.length > 0"
                                ...
                                <p v-for="(citem,cindex) in item.beReplied" :key="cindex"><span
                                        style="color: #4d99de;">@{{citem.user.nickname}}</span>: {{citem.content}}</p>
                            </div>

                            <p style="display:block;font-size: 13px;color: gray;opacity: 0.7;margin-top: 10px;">{{item.time |
                                time}}</p>
                        </div>
                    </div>
                </div>
                ...
                <div v-for="(item,index) in videoCommentList"
                :key="index"
                     style="position: relative;
                padding: 15px 0;font-size: 15px;">
        ...
        <el-col :span="7" :offset="1" style="position:absolute;">
            ...
            <el-row style="margin-top: 10px;margin-left: 12px;">
                <span style="font-size: 14px;">发布时间:
                    <span style="font-size: 14px;">{{videoDetailInfo.data.publishTime | time}}</span>
                </span>
...
<!--            可点击的视频推荐区域-->
            <el-row v-for="(item,index) in simiVideoList" :key="index" style="margin-top: 25px;">
                <el-col :span="12">
                    <el-image :src="item.coverUrl" @click="gomvdetail(item.vid)"
                              style="width: 250px;height: 150px;cursor: pointer;border-radius:10px"></el-image>
                    <!--播放量-->
                ...
                <el-col :span="10" :offset="1" >
                    ...
                    <div style="position: relative;left: 68px;top:-50px">
                        <p style="font-size: 15px;opacity: 0.8;">{{item.durationms/1000 | time}}</p>
